...
- **Receiver taps read only your own network.** `/api/local-aircraft?base=host:port` is the one route whose upstream host comes from the browser. The address must pass `parseTapAddress` (`src/data/tapAddress.js`): loopback, RFC1918, `localhost` or a `.local` name and nothing else, so link-local (including the cloud metadata address) and IPv6 are refused, as is a value carrying a scheme, path, credentials or query. The path is fixed at `/data/aircraft.json`, redirects are refused rather than followed, and both the wait and the body are bounded. Cross-site pages are refused before the rate limiter (`Sec-Fetch-Site` other than `same-origin`/`none`, or a foreign or opaque `Origin`): a page that cannot read the answer could otherwise still map a LAN from the status codes and timing.
