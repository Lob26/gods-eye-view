...
### My Receiver — local ADS-B (September 2026)

- MOVEMENT ▸ **My Receiver (ADS-B)** (`local-aircraft`, token `2`, `src/layers/localAircraft/`) draws aircraft heard by the user's own dump1090-fa / readsb / tar1090 receiver, in magenta (`#ff3ec8`: magenta because #134 chose it for its browser-USB contacts, this shade because the weather radar's heaviest band is pure `#ff00ff`) beside public Flights. Decided in #57: **draw both** — nothing is hidden from Flights, so the same airframe can appear twice. The receiver mark leads, because Flights renders about one poll behind to keep its motion smooth; that offset is correct and must not be corrected by delaying local contacts. Hiding the public mark where the receiver has the plane is a possible later option. Grouped after Military in the Movement group.
- **Route.** `/api/local-aircraft?base=host:port` (`server/providers/localAircraft.js`, development and preview) is the first consumer of the tap contract (#382, `src/data/tapAddress.js`). Order of checks: method (`GET` only, 405), cross-site (`Sec-Fetch-Site` other than `same-origin`/`none`, or a foreign or opaque `Origin`, 403), rate limit (always on, 120 per client and 400 global per minute, 429), address (`parseTapAddress`, 400 with no fallback host). It then reads the fixed path `/data/aircraft.json` with `redirect: 'manual'` (any 3xx is 502), a 4 s timeout that also reaches the body read (504), and a 2 MB cap (502), and relays the body unmodified with `no-store`. An unreachable receiver is a generic 502 whose log names no host. The cross-site gate is the rule #242 proposes as shared; the route should adopt that helper when it lands.
- **Model.** `model.js` normalizes one `aircraft.json` snapshot. Per-aircraft parsing and unit conversion are delegated to `normalizeAdsbLolAircraftState`, because adsb.lol v2 rows are readsb rows; the module owns the envelope (`aircraft`, not `ac`), the receiver-quality fields that normalizer drops (`rssi`, `messages`), and the heard / positioned split: a row with an identity counts as heard, and only a row with a position no older than 30 s (`DEFAULT_MAX_POSITION_AGE_S`, #134's figure) is drawn. An empty or malformed snapshot is zero contacts, never an error.
- **Producers.** The layer reads any object with `getSnapshot({signal})` returning that shape. `createReceiverTapSource` (`source.js`) is the network producer; it re-validates the address in the browser for immediate feedback and reads it on every poll. The browser-USB decoder (#134) is meant to be a second producer with no address and no server route.
- **Lifecycle.** A level-triggered reconcile keyed by ICAO every 2 s: new contacts are added, surviving ones moved in place (same entity, so no flicker), missing ones removed. Contacts are cleared on disable and on an unreachable receiver. Stats: `count` is drawn and `heard` is heard; the row shows `Heard N · drawing M` on its own `info` line under the chip, because in the count column the sentence wrapped the whole row. No address or a rejected one is guidance (`status: 'idle'` with `statusMessage`), a receiver hearing nothing is `empty`, and only a failed read sets `error`. Contacts are non-trackable. Billboards use the shared class silhouettes (`aircraftIcon`, `classifyAircraft`) at the same size as a Flights mark of the same class (`CLASS_SCALE_2D` and Flights' `NearFarScalar(1000, 3.0, 8000000, 0.5)`), so the two layers differ only in colour; rotated by true track (`-track` about `Cartesian3.UNIT_Z`, which puts 0 at north, checked in a browser at all four cardinal headings), clamped to ground when on the ground or without altitude, and never depth-tested.
- **Address.** A row chip, SET RECEIVER / `RX host:port`, prompts for the address; cancel keeps it and an empty answer clears it. It is stored in `localStorage` (`gev-local-aircraft-receiver`, with an in-memory fallback when storage is blocked) and never in layer-state params — the registry disposition is `enabled-only` — because it is a host on the user's network.
- Regression surface: the colocated `src/layers/localAircraft/*.test.mjs`, `src/app/layers/localAircraft.test.mjs` (storage fallback; the address stays out of share links) and `src/tooling/localAircraftProvider.test.mjs` (every refused address reaches no fetch, fixed path, redirect, stalled body, cross-site gate before the limiter).
