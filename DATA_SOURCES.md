...
- **Camera frame content.** Public camera frames can contain people, vehicles, and license plates, so this is the project's stated position on what it does with them: **nothing.** Frames are relayed live from the provider's own public feed and reach the browser exactly as that provider served them — no blurring, redaction, sharpening, upscaling, face or plate detection, or any other processing is applied, and none is planned. `proxyMediaResponse` pipes the upstream body straight through without buffering it, so GEV never holds a frame; the disk cache (`.gev-cache/`) stores feed JSON for other layers and never camera imagery. The proxy relays the upstream's own `Cache-Control` and substitutes `no-store` when the upstream sends none, which means any browser-side retention is the provider's policy rather than this app's. Frames are fetched only from server-registered camera URLs — a client cannot hand the proxy a URL (see [SECURITY.md](SECURITY.md)). This is the same exposure the upstream feed already has in public, viewed through a different window, and it stays inside the line README draws: no named-person search, no face recognition, no tracking individuals. A source pack that needs frames altered before they can be shown is a source pack that shouldn't be added.
